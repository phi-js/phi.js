<template>
  <div class="ec-transaction">
    <slot name="items" :items="items" :total="total">
    ...
    <slot name="loading" v-if="!previewTransaction || providerIsLoading">
      <ui-loading></ui-loading>
    </slot>

    <div class="ec-transaction-result" v-if="isDone">
      <slot v-if="isSuccess" name="success">
        <ui-item icon="g:check" text="Pago recibido"></ui-item>
      </slot>
      <slot v-else name="error">
        ...
        >
          <template #right>
            <ui-icon class="icon-redo" value="g:refresh" @click="reset"></ui-icon>
          </template>
        </ui-item>
      </slot>
    </div>
...
export default {
  ...
  computed: {
    ...
    availableGateways() {
      ...
      let found = this.previewTransaction.gateways.find(
        g => g.id == this.gatewayId
      );
      return found ? [found] : [];
    },
  ...
  methods: {
    ...
    getProviderDefinition(providerName) {
      ...
      return import(`@/modules/ecommerce/providers/${providerName}/index.js`).then(
